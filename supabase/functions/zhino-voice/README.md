...
> **وضعیت فعلی: خاموش (dormant).** صدای مشتری در سایت فقط با موتور خودِ
> مرورگر (Web Speech API) پخش می‌شود — رایگان، بدون کلید API و بدون هیچ
> درخواست شبکه‌ای برای صدا. این تابع، کلید `TTS_API_KEY` و کلید «صدای ابری»
> در پنل مدیریت عمداً سرِ جایشان مانده‌اند تا در صورت نیاز صدای ابری دوباره
> فعال شود؛ ولی هوک `useAssistantSpeech` دیگر هیچ صدایی از اینجا نمی‌گیرد.
...
مسیر صدای فرانت‌اند (امروز):
۱) مناسب‌ترین صدای فارسی دستگاه ← ۲) نبودِ صدای فارسی؟ fallback خودِ مرورگر
با `lang="fa-IR"` ← ۳) فقط اگر موتور واقعاً شکست خورد، پیام کوتاه فارسی.
برای فعال‌کردن دوبارهٔ صدای ابری، کافی است هوک صدا این تابع را صدا بزند
(کدش در تاریخچهٔ git هست) و `TTS_API_KEY` در Secrets باشد.
...
| --- | --- | --- |
| `openai` | OpenAI «gpt-4o-mini-tts» | مسیر قابل بازگردانی؛ فعلاً dormant و خارج از اجرای کاربر |
| `azure` | Azure AI Speech (fa-IR-DilaraNeural) | مسیر قابل بازگردانی؛ کد انتخاب‌پذیر باقی مانده است |
...
### مسیر OpenAI (برای بازگردانی احتمالی)
...
### مسیر Azure (برای بازگردانی احتمالی)
...
**وضعیت فعلی:** کاری لازم نیست و مسیر کاربر این تابع را صدا نمی‌زند. هوک
`useAssistantSpeech` در سایت مستقیماً از `SpeechSynthesis` مرورگر استفاده می‌کند؛
بنابراین متن پاسخ از دستگاه خارج نمی‌شود، هیچ `POST` به `zhino-voice` یا سرویس
TTS انجام نمی‌شود و UI هم همان UI قبلی می‌ماند.

این قرارداد فقط برای بازگشت احتمالی نگه داشته شده است: پس از فعال‌کردن دوبارهٔ
مسیر ابری در هوک، اگر `VITE_SUPABASE_URL` و کلید عمومی پروژه تنظیم باشند، تابع
روی `<VITE_SUPABASE_URL>/functions/v1/zhino-voice` قابل فراخوانی است و کلید
عمومی را در هدرهای استاندارد `Authorization` و `apikey` می‌فرستد.
...
اگر این قرارداد در آینده دوباره به فرانت‌اند وصل شود، در همهٔ این حالت‌ها به صدای مرورگر و سپس
به پیام کوتاه فارسی می‌رسد — این نگاشت برای هر دو سرویس یکی است.
