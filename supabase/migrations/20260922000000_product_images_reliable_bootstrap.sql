-- ZHINO image API v3. Complete, atomic, standalone forward migration.
-- ONLY initial_schema + seed_catalog are required/allowed predecessors.
-- Do not run v1/v2, seed, repair or include-all to install this file.
-- Public bucket is intentional: image bytes are not confidential.
-- No product DML during installation or reruns. Full-row assertions precede COMMIT.
-- DB receipts/retirement are atomic; Storage HTTP deletion is a separate operation.
-- Maintenance window + verified backup required. Never run on Remote without approval.
...
-- Strict source-state gate. No v1/v2 upgrade or ambiguous partial installation.
do $$ declare installed boolean;
begin
  if to_regclass('supabase_migrations.schema_migrations') is null then
    raise exception 'ZHINO: recorded initial_schema and seed_catalog baseline required';
  end if;
  if (select count(*) from supabase_migrations.schema_migrations
      where (version,name) in (('20260912000000','initial_schema'),('20260912000001','seed_catalog'))) <> 2
    or exists(select 1 from supabase_migrations.schema_migrations
      where version not in ('20260912000000','20260912000001','20260922000000')
        or version='20260922000000' and name is distinct from 'product_images_reliable_bootstrap') then
    raise exception 'ZHINO: unsupported migration history; no repair or predecessor replay';
  end if;
  installed := to_regclass('public.product_image_contract') is not null;
  if not installed then
    if to_regclass('public.product_images') is not null
      or to_regclass('public.product_image_cleanup') is not null
      or to_regclass('public.product_image_operations') is not null
      or exists(select 1 from storage.buckets)
      or exists(select 1 from supabase_migrations.schema_migrations where version='20260922000000')
      or exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace
         where n.nspname='public' and (p.proname like '%product_image%' or p.proname='sync_product_primary_image')) then
      raise exception 'ZHINO: first install requires empty gallery/buckets and no image API; nothing changed';
    end if;
  else
    if to_regclass('public.product_images') is null or to_regclass('public.product_image_cleanup') is null
      or to_regclass('public.product_image_operations') is null
      or to_regprocedure('public.product_image_schema_fingerprint()') is null then
      raise exception 'ZHINO: incomplete v3 installation';
    end if;
    lock table public.product_images,public.product_image_cleanup,public.product_image_operations,public.product_image_contract in share row exclusive mode;
    if (select count(*) from public.product_image_contract) <> 1
      or not exists(select 1 from public.product_image_contract where version=3
        and fingerprint=public.product_image_schema_fingerprint()) then
      raise exception 'ZHINO: v3 schema/security drift; refusing automatic repair';
    end if;
    if exists(select 1 from public.products p where
      ...
      or exists(select 1 from public.product_images i where i.product_id=p.id and i.is_primary
        and i.storefront_url is distinct from p.image_url)) then
      raise exception 'ZHINO: existing gallery inconsistent; refusing migration-time data repair';
    end if;
  end if;
end $$;
...
create table if not exists public.product_image_cleanup (
  ...
  last_error_code text check(last_error_code in ('storage_unconfirmed','still_referenced','object_present'))
);
create index if not exists product_image_cleanup_due on public.product_image_cleanup(next_attempt_at,created_at,storage_path) where completed_at is null;
create table if not exists public.product_image_operations (
...
create table if not exists public.product_image_contract (
...
-- Owned v3 indexes. Unknown predecessor/partial tables have already been rejected.
...
as $$ begin
  if new.image_url is not null and not (btrim(new.image_url) like 'images/%' or btrim(new.image_url) like '/images/%'
    or btrim(new.image_url) ~ '^https://[^[:space:]]+$') then raise exception 'invalid_image_url'; end if;
...
create or replace function public.execute_product_image_operation(
...
begin
  ...
  if p_action = 'register' and i.id is not null then raise exception 'image_operation_conflict'; end if;
  ...
  if p_action = 'register' then
    ...
    v_primary := coalesce((p_payload->>'make_primary')::boolean, false) or p.image_url is null or btrim(p.image_url)='';
    ...
    update public.products set image_url = i.storefront_url where id = p.id and image_url is distinct from i.storefront_url;
...
-- Idempotency is per operation, NOT per image. A new request with the same
-- operation ID but different intent/actor fails closed. Replays return the
-- original receipt before checking current product/gallery state.
create or replace function public.manage_product_image(
  p_action text,p_product_id text,p_image_id uuid,p_expected_url text,p_payload jsonb default '{}'::jsonb
) returns jsonb language plpgsql security definer set search_path = pg_catalog, pg_temp
as $$ declare op uuid; req jsonb; receipt public.product_image_operations; result jsonb; previous jsonb;
begin
...
-- No legacy writer may bypass the explicit operation/CAS protocol.
...
as $$ begin raise exception 'image_api_upgrade_required'; end $$;
...
-- Lease selection is atomic and skip-locked. Failed/expired jobs do not
-- monopolize the oldest twenty entries. A crashed worker becomes retryable.
create or replace function public.claim_product_image_cleanup(p_limit integer default 20)
returns table(storage_path text,claim_id uuid) language plpgsql security definer set search_path = pg_catalog, pg_temp
as $$ begin
  if not public.is_admin() then raise exception 'admin_required'; end if;
...
-- HTTP success or a confirmed object-not-found is necessary, but NOT sufficient:
-- check retirement, no live references AND absence from storage.objects again.
-- Network uncertainty/permission errors never turn into a success receipt.
create or replace function public.complete_product_image_cleanup(p_path text,p_claim_id uuid,p_storage_confirmed boolean)
returns boolean language plpgsql security definer set search_path = pg_catalog, pg_temp
as $$ declare job public.product_image_cleanup; reason text;
begin
  if not public.is_admin() then raise exception 'admin_required'; end if;
  perform pg_advisory_xact_lock(hashtextextended('zhino-image-references',0));
  select * into job from public.product_image_cleanup where storage_path=p_path for update;
  if not found or job.claim_id is distinct from p_claim_id then return false; end if;
  if job.completed_at is not null then return true; end if;
  if job.lease_until is null or job.lease_until<=clock_timestamp() then return false; end if;
  if public.product_image_path_referenced(p_path) then reason := 'still_referenced';
  elsif exists(select 1 from storage.objects where bucket_id='product-images' and name=p_path) then reason := 'object_present';
  elsif p_storage_confirmed is distinct from true then reason := 'storage_unconfirmed';
  end if;
  if reason is not null then
    update public.product_image_cleanup set lease_until=null,last_error_code=reason,
      next_attempt_at=clock_timestamp()+make_interval(secs=>least(3600,5*power(2,least(job.attempt_count,10)))::integer)
      where storage_path=p_path;
    return false;
  end if;
  update public.product_image_cleanup set completed_at=clock_timestamp(),lease_until=null,last_error_code=null
    where storage_path=p_path;
  return true;
end $$;
...
alter table public.product_image_operations enable row level security;
alter table public.product_image_contract enable row level security;
revoke all on public.product_images,public.product_image_cleanup,public.product_image_operations,public.product_image_contract from public,anon,authenticated;
do $$ declare t text; cols text;
begin
  foreach t in array array['product_images','product_image_cleanup','product_image_operations','product_image_contract'] loop
...
-- Receipts and the schema contract are never a browser table-read API,
-- even if a later permissive policy/grant is accidentally broadened.
do $$ declare t text;
begin
  foreach t in array array['product_image_operations','product_image_contract'] loop
    execute format('drop policy if exists image_private_read_guard on public.%I',t);
    execute format('create policy image_private_read_guard on public.%I as restrictive for select to anon,authenticated using(false)',t);
  end loop;
end $$;
...
revoke all on function public.complete_product_image_cleanup(text,uuid,boolean) from public,anon,authenticated;
grant execute on function public.complete_product_image_cleanup(text,uuid,boolean) to authenticated;
revoke all on function public.execute_product_image_operation(text,text,uuid,text,jsonb) from public,anon,authenticated;
revoke all on function public.claim_product_image_cleanup(integer) from public,anon,authenticated;
grant execute on function public.claim_product_image_cleanup(integer) to authenticated;
...
-- Persist a catalog-only contract. Application rows are deliberately excluded.
-- A rerun rejects drift (including grants, functions, indexes and policies)
-- BEFORE any persistent DDL; IF NOT EXISTS alone is not validation.
create or replace function public.product_image_schema_fingerprint()
...
$$;
revoke all on function public.product_image_schema_fingerprint() from public,anon,authenticated;
insert into public.product_image_contract(version,fingerprint) values(3,public.product_image_schema_fingerprint()) on conflict do nothing;

-- 5. FINAL ASSERTIONS — failure aborts the transaction BEFORE commit.
do $$ declare r text; t text; f text;
begin
  if not exists(select 1 from public.product_image_contract where version=3
    and fingerprint=public.product_image_schema_fingerprint()) then
    raise exception 'ZHINO: schema contract changed during reexecution; rolling back';
  end if;
  ...
  if exists(select 1 from pg_temp.zhino_images_before b left join public.product_images i on i.id=(b.row_data->>'id')::uuid
    where i.id is null or to_jsonb(i) is distinct from b.row_data) then
  ...
  foreach r in array array['anon','authenticated'] loop
    foreach t in array array['public.product_images','public.product_image_cleanup','public.product_image_operations','public.product_image_contract'] loop
    ...
    foreach f in array array['public.reconcile_product_image(text,text,text)','public.product_image_path_referenced(text)',
      'public.product_image_url_references(text,text)','public.normalize_product_image_url()','public.sync_product_primary_image()',
      'public.execute_product_image_operation(text,text,uuid,text,jsonb)','public.product_image_schema_fingerprint()'] loop
  ...
  foreach f in array array['public.manage_product_image(text,text,uuid,text,jsonb)',
    ...
    'public.complete_product_image_cleanup(text,uuid,boolean)','public.product_image_api_version()',
    'public.claim_product_image_cleanup(integer)'] loop
