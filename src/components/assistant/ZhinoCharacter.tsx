...
export default function ZhinoCharacter({
  ...
  const scheduleBlink = useCallback(() => {
    ...
    blinkTimer.current = window.setTimeout(() => {
      setBlink(true);
      window.setTimeout(() => setBlink(false), 120 + Math.random() * 80);
      scheduleBlink();
    }, next);
  }, []);
  ...
  const scheduleEyeMove = useCallback(() => {
    ...
    eyeTimer.current = window.setTimeout(() => {
      // گاهی نگاه مستقیم، گاهی چپ/راست/بالا
      const presets = [
        { x: 0, y: 0 },
        { x: -2.5, y: -0.5 },
        { x: 2.5, y: -0.5 },
      ...
      // در حالت فکر کردن بیشتر به بالا نگاه می‌کند
  ...
  useEffect(() => {
    ...
    const steps: Array<{ phase: typeof greetingPhase; delay: number }> = [
      { phase: 'enter', delay: 0 },
      { phase: 'smile', delay: 350 },
      { phase: 'wave', delay: 650 },
      { phase: 'product', delay: 1450 },
      { phase: 'settle', delay: 2350 },
    ];
    let idx = 0;
    const run = () => {
      if (idx >= steps.length) {
  ...
  return (
    ...
    >
      ...
      >
        <defs>
          {/* گرادینت سر - ivory تا champagne */}
          <radialGradient id="zc-head" cx="42%" cy="32%" r="78%">
          ...
          {/* سایه سر */}
          <radialGradient id="zc-head-shadow" cx="50%" cy="85%" r="65%">
            <stop offset="0%" stopColor="#71313B" stopOpacity="0.06" />
            <stop offset="100%" stopColor="#71313B" stopOpacity="0" />
          </radialGradient>
          {/* بدن زرشکی */}
          <linearGradient id="zc-body" x1="0%" y1="0%" x2="0%" y2="100%">
          ...
          {/* شکم روشن */}
          <radialGradient id="zc-belly" cx="50%" cy="40%" r="70%">
            <stop offset="0%" stopColor="#FFFEFB" />
            <stop offset="100%" stopColor="#F5EDE2" />
          </radialGradient>
          {/* دست */}
          <linearGradient id="zc-arm" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor="#FFFEFB" />
            <stop offset="100%" stopColor="#E8DCC8" />
          </linearGradient>
          {/* چشم سفید */}
          <radialGradient id="zc-eye-white" cx="35%" cy="30%" r="75%">
            <stop offset="0%" stopColor="#FFFFFF" />
            <stop offset="100%" stopColor="#F5F0E8" />
          </radialGradient>
          {/* عنبیه */}
          <radialGradient id="zc-iris" cx="40%" cy="35%" r="70%">
            <stop offset="0%" stopColor="#8A4A55" />
            <stop offset="35%" stopColor="#71313B" />
            <stop offset="100%" stopColor="#4A1220" />
          </radialGradient>
          {/* محصول */}
          <linearGradient id="zc-product" x1="0%" y1="0%" x2="0%" y2="100%">
            <stop offset="0%" stopColor="#FFFFFF" />
            <stop offset="100%" stopColor="#FAF7F2" />
          </linearGradient>
          {/* سایه نرم */}
          <filter id="zc-soft-shadow" x="-30%" y="-30%" width="160%" height="160%">
            <feDropShadow dx="0" dy="8" stdDeviation="10" floodColor="#2A0B12" floodOpacity="0.18" />
          </filter>
          <filter id="zc-inner-glow" x="-20%" y="-20%" width="140%" height="140%">
            <feDropShadow dx="0" dy="1" stdDeviation="0.5" floodColor="#FFFFFF" floodOpacity="0.9" />
          </filter>
          {/* هایلایت طلایی */}
          <linearGradient id="zc-gold" x1="0%" y1="0%" x2="100%" y2="100%">
        ...
        {!compact && (
          <g className="zc-legs">
            <g className="zc-leg zc-leg-left">
              <path d="M78 176 Q76 195 76 210 Q76 218 84 218 Q92 218 92 210 Q92 195 90 176 Z" fill="url(#zc-arm)" stroke="#D8C9B6" strokeWidth="0.8" />
              <ellipse cx="84" cy="218" rx="12" ry="6" fill="#5A1725" />
              <ellipse cx="84" cy="216.5" rx="12" ry="5.5" fill="url(#zc-body)" />
            </g>
            <g className="zc-leg zc-leg-right">
              <path d="M110 176 Q112 195 112 210 Q112 218 120 218 Q128 218 128 210 Q128 195 126 176 Z" fill="url(#zc-arm)" stroke="#D8C9B6" strokeWidth="0.8" />
              <ellipse cx="120" cy="218" rx="12" ry="6" fill="#5A1725" />
              <ellipse cx="120" cy="216.5" rx="12" ry="5.5" fill="url(#zc-body)" />
            </g>
          </g>
        )}
        ...
        {!compact && (
          <g className="zc-body-group" style={{ transformOrigin: '100px 155px' } as any}>
            ...
            <path
              ...
              fill="url(#zc-body)"
              filter="url(#zc-soft-shadow)"
              className="zc-body-main"
            />
            {/* شکم روشن */}
            <ellipse cx="100" cy="154" rx="28" ry="22" fill="url(#zc-belly)" opacity="0.92" />
            {/* دکمه طلایی */}
            <g className="zc-buttons">
              <circle cx="100" cy="142" r="3.2" fill="url(#zc-gold)" stroke="#B89A62" strokeWidth="0.5" />
              <circle cx="100" cy="154" r="3.2" fill="url(#zc-gold)" stroke="#B89A62" strokeWidth="0.5" />
              <circle cx="100" cy="166" r="3.2" fill="url(#zc-gold)" stroke="#B89A62" strokeWidth="0.5" />
            </g>
            {/* یقه */}
            <path d="M82 118 Q100 124 118 118 Q112 112 100 112 Q88 112 82 118 Z" fill="#FAF7F2" stroke="#E8DCC8" strokeWidth="0.8" />
            {/* خط طلایی دور بدن */}
            <path d="M62 128 Q62 112 100 112 Q138 112 138 128" fill="none" stroke="url(#zc-gold)" strokeWidth="1.2" opacity="0.55" strokeLinecap="round" />
          </g>
        )}
        ...
        {!compact && (
          ...
          >
            ...
            <g className="zc-hand" transform={isThinking ? 'translate(44 102)' : isWaving ? 'translate(32 116)' : 'translate(36 118)'}>
              <ellipse cx="0" cy="0" rx="9" ry="8.5" fill="url(#zc-arm)" stroke="#D8C9B6" strokeWidth="0.8" />
              {/* انگشتان در حالت سلام */}
              {isWaving ? (
                <>
                  <ellipse cx="-3" cy="-7" rx="2.2" ry="3" fill="url(#zc-arm)" stroke="#D8C9B6" strokeWidth="0.6" />
                  <ellipse cx="1.5" cy="-8" rx="2.2" ry="3.2" fill="url(#zc-arm)" stroke="#D8C9B6" strokeWidth="0.6" />
                  <ellipse cx="6" cy="-6.5" rx="2" ry="2.8" fill="url(#zc-arm)" stroke="#D8C9B6" strokeWidth="0.6" />
                </>
              ) : isThinking ? (
                <ellipse cx="2" cy="-2" rx="3" ry="2.5" fill="url(#zc-arm)" />
              ) : null}
            </g>
          </g>
        )}
        ...
        {!compact && showProduct && (
          <g className={cn('zc-arm-group zc-arm-right', isGreeting && greetingPhase === 'product' && 'is-lifted')} style={{ transformOrigin: '132px 134px' } as any}>
            ...
            <g className="zc-product-group" transform="translate(148 150)">
              ...
              {/* جعبه محصول */}
              <g className="zc-product-box" filter="url(#zc-soft-shadow)">
                <rect x="-20" y="-16" width="44" height="32" rx="7" fill="url(#zc-product)" stroke="#71313B" strokeWidth="1.1" />
                {/* نوار زرشکی بالا */}
                <rect x="-20" y="-16" width="44" height="9" rx="7" fill="#71313B" />
                <rect x="-20" y="-9" width="44" height="4" fill="#71313B" />
                {/* دایره طلایی کوچک */}
                <circle cx="0" cy="-11.5" r="3" fill="url(#zc-gold)" />
                {/* متن فارسی */}
                ...
                {/* خط تزئینی پایین */}
                <path d="M-12 12 Q0 14 12 12" stroke="url(#zc-gold)" strokeWidth="0.8" fill="none" opacity="0.7" strokeLinecap="round" />
              </g>
              {/* برق محصول */}
        ...
        >
          {/* سایه پشت سر */}
          <ellipse cx={compact ? 60 : 100} cy={compact ? 68 : 98} rx={compact ? 38 : 52} ry={compact ? 36 : 48} fill="url(#zc-head-shadow)" />
          ...
          <g className="zc-eyes">
            ...
            <g className="zc-eye zc-eye-left" transform={`translate(${compact ? 40 : 76} ${compact ? 58 : 86})`}>
              {/* سفیدی */}
              <ellipse cx="0" cy="0" rx={compact ? 10 : 15} ry={compact ? 12 : 17} fill="url(#zc-eye-white)" stroke="#E8DCC8" strokeWidth="0.8" />
              {/* سایه داخل چشم */}
              ...
              <g className="zc-pupil-group" style={{ transform: `translate(${eyeOffset.x}px, ${eyeOffset.y * eyeScaleY}px) scaleY(${eyeScaleY})` } as any}>
                <circle cx="0" cy="0.5" r={compact ? 5.5 : 7.5} fill="url(#zc-iris)" />
              ...
              {/* پلک بالا - برای پلک زدن */}
              <g className={cn('zc-eyelid zc-eyelid-top', blink && 'is-closed')}>
                <path d={`M-${compact ? 10 : 15} -${compact ? 2 : 3} Q0 -${compact ? 14 : 18} ${compact ? 10 : 15} -${compact ? 2 : 3} Q${compact ? 8 : 12} 0 -${compact ? 10 : 15} -${compact ? 2 : 3}`} fill="url(#zc-head)" stroke="#E8DCC8" strokeWidth="0.5" />
              </g>
              {/* مژه‌های ظریف */}
          ...
          {/* کلاه کوچک ژینو - المان برند */}
          {!compact && (
            <g className="zc-hat" transform="translate(100 42)">
              <ellipse cx="0" cy="0" rx="14" ry="5" fill="#5A1725" opacity="0.9" />
              <path d="M-12 -1 Q-10 -12 0 -14 Q10 -12 12 -1 Z" fill="url(#zc-body)" stroke="#4A1220" strokeWidth="0.8" />
              <circle cx="0" cy="-12" r="2.5" fill="url(#zc-gold)" />
            </g>
          )}
        </g>

        {/* افکت درخشش هنگام سلام */}
        {isGreeting && greetingPhase === 'smile' && !compact && (
          <g className="zc-sparkles">
            <circle cx="42" cy="62" r="1.5" fill="url(#zc-gold)" opacity="0.8">
            ...
            <circle cx="158" cy="68" r="1.2" fill="url(#zc-gold)" opacity="0.7">
