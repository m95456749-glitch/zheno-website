 ...
 * فقط برای نمایشِ محصولاتِ صحنه به کار می‌رود — تصمیمِ پاسخ و اتصال به
 * داده‌های فروشگاه همچنان همان مسیر همیشگی خودشان را می‌روند.
 *
 * نکته: واژه‌های درونیِ سامانه (از جمله نامِ فهرستِ کالاها) در این فایل
 * نمی‌آیند — خط‌مشیِ verify:supabase می‌گوید این سه فایلِ گفتگو حتی یک
 * واژهٔ فنی هم نداشته باشند؛ واژه‌های بالا همان‌هایی‌اند که مشتری واقعاً
 * می‌گوید. پرسش‌هایی هم که اینجا نباشند بی‌پاسخ نمی‌مانند: هر وقت پاسخِ
 * ربات محصولی معرفی کند، صحنه خودش همراهِ پاسخ باز می‌شود.
 */
const PRODUCT_INTENT = /محصول|کالا|طعم|قیمت|قيمت|موجود|قفسه|ویترین/u;
